...
### 컴포넌트 개발 시
- [ ] **공통 컴포넌트 우선 사용**: 기존 공통 컴포넌트 (Button, Input, Icons 등)를 먼저 확인하고 사용
- [ ] TypeScript 타입 정의
- [ ] Props interface 정의
- [ ] Storybook 스토리 작성
- [ ] 단위 테스트 작성 (텍스트나 변경 가능한 내용에 의존하지 않고 느슨하고 변경 가능성 적은 항목 중심으로 작성)
- [ ] 접근성 (키보드 네비게이션)

### 테스트 코드 작성 원칙
- **느슨한 결합**: 텍스트, 라벨 등 변경 가능한 내용에 의존하지 않음
- **DOM 구조 중심**: 컴포넌트의 기본 구조와 기능에 집중
- **역할과 행동 테스트**: 컴포넌트가 수행해야 하는 역할과 행동 검증
- **변경 가능성 최소화**: 구현 세부사항이 아닌 사용자 관점에서 테스트
